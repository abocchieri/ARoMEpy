...
def pyarome(
    ...
    true_anom = 2.0 * np.arctan(
        np.sqrt((1.0 + ecc) / (1.0 - ecc)) * np.tan(ecc_anom / 2.0)
    )
    radius_vec = aRs * (1.0 - ecc**2) / (1.0 + ecc * np.cos(true_anom))
    node = lda  # this is np.pi+lda in the C-function?!?!
    true_lat = omega + ke.trueAnomaly(t)

    x_ = radius_vec * (
        -np.cos(node) * np.cos(true_lat) + np.sin(node) * np.sin(true_lat) * np.cos(i)
    )
    y_ = radius_vec * (
        -np.sin(node) * np.cos(true_lat) - np.cos(node) * np.sin(true_lat) * np.cos(i)
    )
...
def setGaussfit_a0(dic, lim=20):
    ...
    integral = (
        ...
        * dic["sigma0"]
        * np.sqrt(np.pi)
        * integrate.quad(funcAmp_a0, 0.0, 1.0, dic, lim)[0]
    )
    return integral
...
def funcAmp_a0(x, dic):
    ...
    sig2 = dic["sigma0"] ** 2 + dic["beta0"] ** 2 + dic["zeta"] ** 2 / 2.0
    mu = np.sqrt(1 - x**2)
    smu = np.sqrt(mu)
...
def arome_get_RM_CCF_e(dic):
    ...
    if dic["zeta"] > 0.0:
        return -0.5 / den * (2.0 * bs2 / (bs2 + bpR2)) ** (3.0 / 2) * f * vp * np.exp(
            -(vp**2) / (2.0 * (bs2 + bpR2))
        ) - 0.5 / den * (2.0 * bs2 / (bs2 + bpT2)) ** (3.0 / 2) * f * vp * np.exp(
            -vp * vp / (2.0 * (bs2 + bpT2))
        )
    # without macro turbulence
        ...
        return (
            -1.0
            / den
            * (2.0 * bs2 / (bs2 + bpR2)) ** (3.0 / 2)
            * f
            * vp
            * np.exp(-vp * vp / (2.0 * (bs2 + bpR2)))
        )
